...
import type { TCategory, TCategoryResponse, TUpdateCategoryPayload } from "@/types/category.type";
...
export function UpdateCategoryModal({ open, category, onClose }: Props) {
  const { mutate, isPending } = useMutate<TCategoryResponse, TUpdateCategoryPayload>(
    "PATCH",
    "/categories",
    {
  ...
  function onSubmit(values: FormValues) {
    if (!category) return;
    mutate({ id: category._id, body: values });
  }
      ...
      >
        <DialogHeader className="flex-row items-center px-6 py-4 border-b border-white/10 gap-0">
          <div className="flex items-center gap-2">
            ...
            <DialogTitle className="text-white text-base">Update Category</DialogTitle>
          </div>
        </DialogHeader>

        <form id="update-category-form" onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
