...
export function CreateProductModal({ open, categories, onClose }: Props) {
  ...
  const { mutate, isPending } = useMutation<TProductResponse, unknown, FormData>({
    mutationFn: (fd) => apiPostFormData<TProductResponse>("/products", fd),
    onSuccess: () => {
      ...
      >
        <DialogHeader className="flex-row items-center px-6 py-4 border-b border-white/10 gap-0 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <DialogTitle className="text-white text-base">Create Product</DialogTitle>
          </div>
        </DialogHeader>

        <div className="overflow-y-auto flex-1">
          <form id="create-product-form" onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
            {/* Image upload */}
              ...
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-xl border border-white/10 bg-white/5 flex items-center justify-center shrink-0 overflow-hidden">
                  {previewUrl
                    ...
                    : <PackagePlus size={20} className="text-slate-600" />
                  }
                </div>
                <div className="flex items-center gap-2">
                  <input ref={fileInputRef} type="file" accept="image/*" onChange={handleFileChange} className="hidden" id="create-product-image" />
                  <label htmlFor="create-product-image" className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg border border-white/10 text-slate-300 text-xs font-medium cursor-pointer hover:bg-white/5 transition-colors">
                    <Upload size={12} />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <input {...register("name")} placeholder="Product name" className="w-full px-3.5 py-2.5 rounded-lg bg-white/5 border border-white/10 text-white placeholder:text-slate-500 text-sm outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 transition-all" />
                {errors.name && <p className="text-xs text-red-400">{errors.name.message}</p>}
              </div>
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-slate-300">SKU <span className="text-red-400">*</span></label>
                <input {...register("sku")} placeholder="e.g. PRD-001" className="w-full px-3.5 py-2.5 rounded-lg bg-white/5 border border-white/10 text-white placeholder:text-slate-500 text-sm outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 transition-all" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input {...register("purchasePrice")} type="number" step="0.01" min="0" placeholder="0.00" className="w-full pl-7 pr-3.5 py-2.5 rounded-lg bg-white/5 border border-white/10 text-white placeholder:text-slate-500 text-sm outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 transition-all" />
                </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500 text-sm">$</span>
                  <input {...register("sellingPrice")} type="number" step="0.01" min="0" placeholder="0.00" className="w-full pl-7 pr-3.5 py-2.5 rounded-lg bg-white/5 border border-white/10 text-white placeholder:text-slate-500 text-sm outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 transition-all" />
                </div>
            ...
            <div className="space-y-1.5">
              ...
              <input {...register("stockQuantity")} type="number" min="0" placeholder="0" className="w-full px-3.5 py-2.5 rounded-lg bg-white/5 border border-white/10 text-white placeholder:text-slate-500 text-sm outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 transition-all" />
        ...
        <div className="flex gap-3 px-6 py-4 border-t border-white/10 shrink-0">
          ...
          <button type="submit" form="create-product-form" disabled={isPending} className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg bg-violet-600 hover:bg-violet-500 disabled:opacity-60 disabled:cursor-not-allowed text-white text-sm font-medium transition-colors">
