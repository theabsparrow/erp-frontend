...
export function UserTable({ data, isLoading, deletingId, onView, onEdit, onDelete }: Props) {
  const columns = [
    col.display({
      id: "user",
      header: "User",
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {table.getRowModel().rows.map((row, i) => (
            <tr
              key={row.id}
              className={`border-b border-white/5 hover:bg-white/[0.03] transition-colors ${i % 2 === 0 ? "" : "bg-white/[0.02]"}`}
            >
